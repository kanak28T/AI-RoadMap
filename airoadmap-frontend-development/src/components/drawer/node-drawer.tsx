...
function QuizSection({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setQuizError(err.message || "Failed to submit answers.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (!quiz && !loading) {
    return (
      <div className="space-y-3">
        {quizError && (
          <p className="text-xs text-rose-600 bg-rose-50 p-2 rounded border border-rose-200">{quizError}</p>
        )}
  ...
  return (
    <div className="space-y-4">
      ...
      {quiz.questions.map((q: QuizQuestion, qi: number) => {
        const correctIdx = q.answerIndex ?? (q as any).correctIndex ?? 0;
        return (
          <div key={qi} className="space-y-2">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      !submitted && isSelected && 'bg-indigo-50 border-indigo-400 text-indigo-900 font-medium',
                      !submitted && !isSelected && 'bg-white border-slate-200 hover:bg-slate-50 text-slate-800'
                    )}
                  >
                    {opt}
                  </button>
            ...
            {submitted && result?.explanations && result.explanations[qi] && (
              <p className="text-xs text-slate-600 italic pl-2 border-l-2 border-slate-200 mt-1">
        ...
        <div className="space-y-2">
          <div className={cn(
            ...
            {result?.passed
              ...
              : `Score: ${result?.score}% — Review resources or try again!`}
          </div>
          {!result?.passed && (
...
export function NodeDrawer({
  ...
  const handleReroute = async () => {
    ...
    setRerouteError(null);
    try {
      await onReroute(node.id, stuckContext.trim() || undefined);
      onMarkStuck(node.id);
      setShowStuckInput(false);
      setStuckContext('');
    } catch (err: any) {
      console.error('Failed to add bridge topics:', err);
      setRerouteError(err.message || 'Failed to generate bridge topics. Please try again.');
    } finally {
      ...
      {node && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 h-full w-[480px] max-w-full bg-white border-l border-slate-200 shadow-2xl z-30 flex flex-col"
          >
            {/* Header */}
            <div className="flex items-start justify-between p-5 border-b border-slate-100">
              <div className="flex-1 min-w-0 pr-4">
                ...
                <h2 className="text-lg font-bold text-slate-900 leading-snug">
                  {node.title || (node as any).label}
                </h2>
                <div className="flex items-center gap-1 mt-1 text-xs text-slate-500">
                  <Clock className="w-3.5 h-3.5" />
                  {node.estimatedHours || 2}h estimated
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              {/* Section 1 – Why this is in your path */}
              <div className="p-4 bg-indigo-50/70 border border-indigo-100 rounded-xl">
                ...
                <p className="text-sm text-slate-700 leading-relaxed">
                  {node.whyRecommended || (node as any).description || 'Build foundational knowledge and practical competence in this core milestone.'}
                </p>
              </div>
                  ...
                  {node.resources && node.resources.length > 0 ? (
                    node.resources.map((res, idx) => (
                      <a
                        ...
                        className="flex items-start gap-3 p-3 bg-white border border-slate-200 rounded-xl hover:border-indigo-300 hover:shadow-xs transition-all group"
                      >
                        <div className="flex-shrink-0 mt-0.5">
                          <SourceIcon source={res.source || 'Docs'} />
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <Badge variant="outline" className="text-[11px] py-0 px-1.5 text-slate-600">
                              {res.source || 'Documentation'}
                            </Badge>
                            {res.isVerified && (
                        ...
                        <ExternalLink className="w-3.5 h-3.5 text-slate-400 flex-shrink-0 mt-0.5 group-hover:text-indigo-600" />
                      </a>
                    ))
                  ) : (
              ...
              {node.status !== 'COMPLETED' && (
                <div className="border border-rose-200 bg-rose-50/60 rounded-xl p-4 space-y-3">
                  ...
                  {rerouteError && (
                    <p className="text-xs text-rose-800 bg-rose-100 p-2 rounded border border-rose-300">
                      {rerouteError}
                    </p>
                  )}
                  ...
                  {!showStuckInput ? (
                    <Button
                      variant="outline"
                      size="sm"
                      className="w-full border-rose-300 bg-white text-rose-700 hover:bg-rose-100"
                    ...
                    <div className="space-y-2">
                      <textarea
                        ...
                        className="w-full text-sm text-slate-900 placeholder:text-slate-400 border border-rose-200 rounded-lg p-2.5 resize-none h-20 focus:outline-none focus:ring-2 focus:ring-rose-400 bg-white"
                      />
