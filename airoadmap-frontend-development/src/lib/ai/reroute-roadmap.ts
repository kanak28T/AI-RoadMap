...
function buildFallbackBridge(input: RerouteRoadmapInput): GeneratedRoadmap {
  const { currentGraph, stuckNodeId } = input;
  const stuckNode = currentGraph.nodes.find(n => n.id === stuckNodeId);
  if (!stuckNode) return currentGraph;

  const parentIds = getParentIds(currentGraph.edges as RoadmapEdge[], stuckNodeId);
  const bridgeBaseId = `bridge-${stuckNodeId}`;
  const bridgeOneId = `${bridgeBaseId}-foundation`;
  const bridgeTwoId = `${bridgeBaseId}-practice`;

  const bridgeNodes = [
    {
      id: bridgeOneId,
      title: `Foundations for ${stuckNode.title}`,
      type: "bridge" as const,
      level: stuckNode.level,
      estimatedHours: 2,
      whyRecommended: `This bridge topic covers the prerequisite ideas needed before moving on to "${stuckNode.title}".`,
      searchKeywords: [stuckNode.title.toLowerCase(), "prerequisites", "foundation"],
      prerequisites: parentIds.length > 0 ? parentIds : [],
    },
    {
      id: bridgeTwoId,
      title: `Apply ${stuckNode.title} in practice`,
      type: "bridge" as const,
      level: stuckNode.level,
      estimatedHours: 2,
      whyRecommended: `A short practice step reduces friction and makes the main topic more approachable.`,
      searchKeywords: [stuckNode.title.toLowerCase(), "practice", "guided exercise"],
      prerequisites: [bridgeOneId],
    },
  ];
  ...
  const newEdges: RoadmapEdge[] = [
    ...parentIds.map(pid => ({ id: edgeId(pid, bridgeOneId), source: pid, target: bridgeOneId })),
    { id: edgeId(bridgeOneId, bridgeTwoId), source: bridgeOneId, target: bridgeTwoId },
    { id: edgeId(bridgeTwoId, stuckNodeId), source: bridgeTwoId, target: stuckNodeId },
  ];

  const updatedNodes = currentGraph.nodes.map(node =>
    node.id !== stuckNodeId ? node : {
      ...node,
      prerequisites: [...node.prerequisites.filter(p => !parentIds.includes(p)), bridgeTwoId],
    }
  );
  ...
  return {
    ...
    totalEstimatedHours: nodesWithBridges.reduce((sum, node) => sum + node.estimatedHours, 0),
  };
}
...
export async function rerouteRoadmap(
  ...
  if (!process.env.GROQ_API_KEY?.trim()) {
    console.warn("[rerouteRoadmap] GROQ_API_KEY is missing; using built-in fallback bridge route.");
    return buildFallbackBridge(input);
  }
...
- Return ONLY JSON, no markdown, no code fences.`;
  ...
  console.warn("[rerouteRoadmap] Falling back to a deterministic bridge plan after AI validation failures.");
  return buildFallbackBridge(input);
}
