...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Step 2: Enrich with verified resources ──────────────────────────────
    ...
    const isAuthError = /invalid api key|unauthorized|401|403/i.test(message);

    return NextResponse.json(
      {
        error: isAuthError
          ? "AI provider authentication failed. Please verify your AI API key in environment variables."
          : message,
      },
